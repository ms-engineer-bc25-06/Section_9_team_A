...
def upgrade() -> None:
    ...
    # アカウント状態関連のカラムを追加
    op.add_column(
        "users", sa.Column("is_premium", sa.Boolean(), default=False, nullable=True)
    )
    op.add_column(
        "users", sa.Column("is_admin", sa.Boolean(), default=False, nullable=True)
    )
    ...
    op.add_column(
        "users",
        sa.Column(
            "subscription_status", sa.String(length=50), default="free", nullable=True
        ),
    )
    op.add_column(
        "users",
    ...
    op.add_column(
        "users",
        sa.Column("monthly_voice_minutes", sa.Integer(), default=0, nullable=True),
    )
    op.add_column(
        "users",
        sa.Column("monthly_analysis_count", sa.Integer(), default=0, nullable=True),
    )
    op.add_column(
