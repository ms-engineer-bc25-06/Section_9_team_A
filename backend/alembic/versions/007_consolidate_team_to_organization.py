...
def upgrade():
    ...
    try:
        # owner_idカラムの追加
        op.add_column(
            "organizations", sa.Column("owner_id", sa.Integer(), nullable=True)
        )
    ...
    try:
        ...
        op.create_foreign_key(
            "fk_voice_sessions_organization_id",
            "voice_sessions",
            "organizations",
            ["organization_id"],
            ["id"],
...
def downgrade():
    ...
    try:
        op.drop_constraint(
            "fk_voice_sessions_organization_id", "voice_sessions", type_="foreignkey"
        )
    ...
    try:
        op.drop_constraint(
            "fk_chat_rooms_organization_id", "chat_rooms", type_="foreignkey"
        )
