...
from app.schemas.participant_management import ParticipantRole, ParticipantStatus
...
async def test_participant_management():
    ...
    try:
        ...
        # 2. 参加者リスト取得テスト
        print("\n2. 参加者リスト取得テスト")
        participants = await participant_management_service.get_session_participants(session.id)
        ...
        await participant_management_service.update_participant_status(
            session.id, user_to_update.id, ParticipantStatus.MUTED
        )
        ...
        await participant_management_service.change_participant_role(
            session.id, user_to_update.id, ParticipantRole.MODERATOR
        )
        print(f"  ユーザー {user_to_update.username} の役割をモデレーターに変更")
        
        # 6. 更新後の参加者リスト確認
        print("\n6. 更新後の参加者リスト確認")
        updated_participants = await participant_management_service.get_session_participants(session.id)
        ...
        # 7. 参加者退出テスト
        print("\n7. 参加者退出テスト")
        await participant_management_service.leave_session(session.id, user_to_update.id)
        print(f"  ユーザー {user_to_update.username} がセッションから退出")
        
        # 8. 最終参加者リスト確認
        print("\n8. 最終参加者リスト確認")
        final_participants = await participant_management_service.get_session_participants(session.id)
        ...
        # 9. セッション統計情報テスト
        print("\n9. セッション統計情報テスト")
        stats = await participant_management_service.get_session_statistics(session.id)
        print(f"  総参加者数: {stats.total_participants}")
        print(f"  現在の参加者数: {stats.current_participants}")
        print(f"  平均音声レベル: {stats.average_audio_level:.2f}")
        print(f"  総発話時間: {stats.total_speak_time:.2f}秒")
