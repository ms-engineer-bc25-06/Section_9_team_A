...
class SessionStateManager:
    ...
    async def create_session(
        self, session_id: str, host_user: User
    ) -> SessionStateInfo:
        """セッションを作成"""
        try:
            # 既存セッションをチェック
    ...
    async def end_session(self, session_id: str, user_id: int) -> SessionStateInfo:
        ...
        try:
            ...
            session_state.duration = (
                (session_state.ended_at - session_state.started_at).total_seconds()
            ...
            # 録音を停止
            if session_state.recording.state in [
            ...
            # コールバックを実行
            await self._execute_state_change_callbacks(
    ...
    async def add_participant(
        self, session_id: str, user: User, connection_id: str
    ) -> SessionParticipant:
        """参加者を追加"""
        try:
            session_state = self.active_sessions.get(session_id)
    ...
    async def update_participant_state(
        self, session_id: str, user_id: int, state: ParticipantState, **kwargs
    ) -> bool:
        """参加者状態を更新"""
        try:
            session_state = self.active_sessions.get(session_id)
            ...
            logger.debug(
                f"Updated participant {user_id} state to {state} in session {session_id}"
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to update participant state in session {session_id}: {e}"
            )
            return False
    ...
    async def update_transcription_stats(self, session_id: str, **kwargs) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to update transcription stats for session {session_id}: {e}"
            )
            return False
    ...
    async def update_analytics_progress(
        self, session_id: str, progress_percentage: float, current_phase: str, **kwargs
    ) -> bool:
        """分析進捗を更新"""
        try:
            session_state = self.active_sessions.get(session_id)
            ...
            logger.error(
                f"Failed to update analytics progress for session {session_id}: {e}"
            )
            return False
    ...
    async def _execute_state_change_callbacks(
        ...
        try:
            ...
            for session_id, session_state in self.active_sessions.items():
                ...
                if (
                    ...
                    and (current_time - session_state.started_at).total_seconds()
                ...
                # 長時間アクティブでないセッション
                if (
                    session_state.last_update
                    and (current_time - session_state.last_update).total_seconds()
                    > self.cleanup_interval
                ):
                    # 参加者がいないセッション
                    active_participants = [
                        p
                        for p in session_state.participants.values()
...
class SessionStateService:
    """セッション状態サービスクラス（SessionStateManagerのラッパー）"""

    def __init__(self):
        self._manager = session_state_manager

    async def create_session(
        self, session_id: str, host_user: User
    ) -> SessionStateInfo:
        """セッションを作成"""
        return await self._manager.create_session(session_id, host_user)

    async def start_session(self, session_id: str, user_id: int) -> SessionStateInfo:
        """セッションを開始"""
        return await self._manager.start_session(session_id, user_id)

    async def pause_session(self, session_id: str, user_id: int) -> SessionStateInfo:
        """セッションを一時停止"""
        return await self._manager.pause_session(session_id, user_id)

    async def resume_session(self, session_id: str, user_id: int) -> SessionStateInfo:
        """セッションを再開"""
        return await self._manager.resume_session(session_id, user_id)

    async def end_session(self, session_id: str, user_id: int) -> SessionStateInfo:
        """セッションを終了"""
        return await self._manager.end_session(session_id, user_id)

    async def add_participant(
        self, session_id: str, user: User, connection_id: str
    ) -> SessionParticipant:
        """参加者を追加"""
        return await self._manager.add_participant(session_id, user, connection_id)

    async def remove_participant(self, session_id: str, user_id: int) -> bool:
        """参加者を削除"""
        return await self._manager.remove_participant(session_id, user_id)

    async def update_participant_state(
        self, session_id: str, user_id: int, state: ParticipantState, **kwargs
    ) -> bool:
        """参加者状態を更新"""
        return await self._manager.update_participant_state(
            session_id, user_id, state, **kwargs
        )

    async def start_recording(self, session_id: str, user_id: int, **kwargs) -> bool:
        """録音を開始"""
        return await self._manager.start_recording(session_id, user_id, **kwargs)

    async def stop_recording(self, session_id: str, user_id: int) -> bool:
        """録音を停止"""
        return await self._manager.stop_recording(session_id, user_id)

    async def start_transcription(self, session_id: str, **kwargs) -> bool:
        """転写を開始"""
        return await self._manager.start_transcription(session_id, **kwargs)

    async def update_transcription_stats(self, session_id: str, **kwargs) -> bool:
        """転写統計を更新"""
        return await self._manager.update_transcription_stats(session_id, **kwargs)

    async def start_analytics(self, session_id: str, **kwargs) -> bool:
        """分析を開始"""
        return await self._manager.start_analytics(session_id, **kwargs)

    async def update_analytics_progress(
        self, session_id: str, progress_percentage: float, current_phase: str, **kwargs
    ) -> bool:
        """分析進捗を更新"""
        return await self._manager.update_analytics_progress(
            session_id, progress_percentage, current_phase, **kwargs
        )

    async def get_session_state(self, session_id: str) -> Optional[SessionStateInfo]:
        """セッション状態を取得"""
        return await self._manager.get_session_state(session_id)

    async def get_all_active_sessions(self) -> List[SessionStateInfo]:
        """すべてのアクティブセッションを取得"""
        return await self._manager.get_all_active_sessions()

    async def get_session_participants(
        self, session_id: str
    ) -> List[SessionParticipant]:
        """セッション参加者を取得"""
        return await self._manager.get_session_participants(session_id)

    async def get_session_history(self, session_id: str) -> List[SessionStateInfo]:
        """セッション履歴を取得"""
        return await self._manager.get_session_history(session_id)

    async def register_state_change_callback(self, session_id: str, callback: Callable):
        """状態変更コールバックを登録"""
        await self._manager.register_state_change_callback(session_id, callback)
