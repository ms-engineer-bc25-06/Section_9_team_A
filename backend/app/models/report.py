...
class Report(Base):
    ...
    # 基本情報
    report_id = Column(String(100), unique=True, nullable=False, index=True, comment="レポートID（UUID）")
    comparison_id = Column(String(100), nullable=False, index=True, comment="比較分析ID")
    report_type = Column(String(50), nullable=False, comment="レポートタイプ（comparison, team, industry）")
    ...
    # レポート設定
    report_format = Column(String(20), default="pdf", comment="レポート形式（pdf, html, json）")
    include_charts = Column(Boolean, default=True, comment="チャートを含めるか")
    include_recommendations = Column(Boolean, default=True, comment="推奨事項を含めるか")
    language = Column(String(10), default="ja", comment="レポート言語")
    
    # 生成情報
    generated_by = Column(Integer, ForeignKey("users.id"), comment="生成者ID")
    generated_at = Column(DateTime(timezone=True), server_default=func.now(), comment="生成日時")
...
class ReportTemplate(Base):
    ...
    template_code = Column(String(50), unique=True, nullable=False, comment="テンプレートコード")
    display_name = Column(String(200), nullable=False, comment="表示名")
    description = Column(Text, comment="テンプレートの説明")
    
    # テンプレート設定
    template_type = Column(String(50), nullable=False, comment="テンプレートタイプ（comparison, team, industry）")
...
class ReportExport(Base):
    ...
    # エクスポート情報
    report_id = Column(Integer, ForeignKey("reports.id"), nullable=False, comment="レポートID")
    export_format = Column(String(20), nullable=False, comment="エクスポート形式")
    export_type = Column(String(50), comment="エクスポートタイプ（download, email, share）")
    ...
    # エクスポート状況
    status = Column(String(50), default="processing", comment="ステータス（processing, completed, failed）")
    error_message = Column(Text, comment="エラーメッセージ")
    
    # エクスポート情報
    exported_by = Column(Integer, ForeignKey("users.id"), comment="エクスポート実行者ID")
    exported_at = Column(DateTime(timezone=True), server_default=func.now(), comment="エクスポート日時")
...
class ReportShare(Base):
    ...
    # 共有設定
    report_id = Column(Integer, ForeignKey("reports.id"), nullable=False, comment="レポートID")
    shared_by = Column(Integer, ForeignKey("users.id"), nullable=False, comment="共有者ID")
    ...
    # 共有設定
    share_type = Column(String(50), nullable=False, comment="共有タイプ（user, team, public, link）")
    permission_level = Column(String(50), default="view", comment="権限レベル（view, comment, edit）")
