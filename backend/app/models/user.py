...
class User(Base):
    ...
    last_logout_at = Column(DateTime(timezone=True), nullable=True)  # ログアウト時刻
    ...
    # user_roles = relationship("UserRole", back_populates="user")  # 一時的に無効化（リレーションシップエラー回避）
    # profile = relationship("UserProfile", uselist=False, back_populates="user")  # 一時的に無効化（UserProfileモデル未実装）
